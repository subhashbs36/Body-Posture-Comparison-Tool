...
from pose_module2 import poseDetector
...
class PoseComparisonApp(QMainWindow):
    ...
    def initUI(self):
        ...
        # Create labels to display the highest angles
        self.highest_angle_labels = {
            "Shoulder": QLabel("Shoulder: 0", self),
            "Elbow": QLabel("Elbow: 0", self),
            "Wrist": QLabel("Wrist: 0", self),
            "Knee": QLabel("Knee: 0", self),
            "Ankle": QLabel("Ankle: 0", self),
        }

        for label in self.highest_angle_labels.values():
            label.setAlignment(Qt.AlignCenter)
            label.setStyleSheet("font-size: 12px")

        self.layout.addLayout(self.buttons_layout)

        # Create a QVBoxLayout for the labels
        labels_layout = QVBoxLayout()

        for label in self.highest_angle_labels.values():
            labels_layout.addWidget(label)

        # Create a QHBoxLayout for the labels and Matplotlib canvas
        labels_and_canvas_layout = QHBoxLayout()

        # Add labels layout to the labels and canvas layout

        # Add labels layout to the labels and canvas layout with a stretch factor
        labels_and_canvas_layout.addLayout(labels_layout, 1)

        # Matplotlib integration
        self.figure = Figure()
        self.canvas = FigureCanvas(self.figure)
        labels_and_canvas_layout.addWidget(self.canvas, 3)  # Adjust the stretch factor as needed

        # Add the labels and canvas layout to the main layout
        self.layout.addLayout(labels_and_canvas_layout)
    ...
    def update_frame(self):
        if not self.paused and self.benchmark_cam is not None and self.user_cam is not None:
            try:
                ...
                # Assuming you have access to the pose detector instance
                highest_angles = self.detector_1.get_highest_angles()

                for label_name, label in self.highest_angle_labels.items():
                    label.setText(f"{label_name}: {highest_angles[label_name]}")

                # Add this print statement to check the values of highest_angles
                print("Highest Angles:", highest_angles)
